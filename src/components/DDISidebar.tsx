...
interface SidebarProps {
    ddiInfo: {
        "drugA": DrugSearchResult;
        "drugB": DrugSearchResult;
        "ddi": { [key: string]: DDIResult }; // 使用索引签名来表示任意键的 DDIResult 对象
    };
}

export const DDISidebar: React.FC<SidebarProps> = ({ddiInfo}) => {
                ...
                <div>
                    ...
                    {Object.keys(ddiInfo.ddi).map((ddiType, index) => (
                        <div key={index}>
                            ...
                            <button
                                onClick={() => handleYesDDIButtonClick(ddiInfo.ddi[ddiType].description)}>点击查询LLM分析
                            </button>
